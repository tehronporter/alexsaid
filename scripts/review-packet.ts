...
const entries = stage === "first"
  ? candidates.filter((candidate) => candidate.status === "candidate").slice(0, limit).map((candidate) => {
  ...
  : ledger.records.filter((record) => stage === "second" ? record.status === "in_review" && record.verification.firstPass && !record.verification.secondPass : record.status === "verified" && !record.verification.blindAudit).slice(0, limit).map((record) => {
