...
export default async function SourcePage({ params }: Props) {
  ...
  return (
    <main className="page-wrap">
      <Link href={`/q/${quote.id}`} className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-white/70 hover:text-white"><ArrowLeft className="size-4" />Back to quote</Link>
      <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-3xl border border-white/15 p-7 sm:p-10"><Badge className="bg-black text-white">{quote.primaryCategory}</Badge><span className="display-type mt-12 block text-8xl leading-[0.4]">“</span><blockquote className={`display-type mt-8 ${quoteDisplaySize(quote.text, "panel")} uppercase leading-[0.9]`}>{quote.text}</blockquote><p className="mt-7 text-sm font-extrabold uppercase tracking-[0.1em]">{quote.author}</p></section>
