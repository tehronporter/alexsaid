  ...
  return (
    <div className="space-y-9">
      <label className="relative block">
        ...
        }} placeholder="Search quotes, topics, or sources…" className="h-14 border-white/15 bg-black/80 pl-12 text-white placeholder:text-white/40" />
      </label>
      ...
      {query.trim() ? (
        <section aria-live="polite">
          ...
          {results.length ? <div className="grid gap-3 lg:grid-cols-2">{results.map((quote) => <QuoteListCard key={quote.id} quote={quote} />)}</div> : <Card className="content-card p-8 text-center"><Search className="mx-auto mb-3 size-7 text-white/40" /><p className="font-semibold">No matching ideas yet.</p><p className="mt-1 text-sm text-white/50">Try a topic like sales, offers, or confidence.</p></Card>}
        </section>
      ) : (
        <>
          <section>
            <div className="mb-4 flex items-center justify-between"><h2 className="text-sm font-extrabold uppercase tracking-[0.14em]">Topics</h2><span className="text-xs text-white/45">Browse the library</span></div>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
              {categories.slice(0, 10).map((category) => {
                const Icon = iconMap[category] ?? Sparkles;
                const count = quotes.filter((quote) => quote.primaryCategory === category).length;
                return <Link key={category} href={`/discover?topic=${encodeURIComponent(category)}`} onClick={() => trackProductEvent("topic_opened", { category })} className="group rounded-2xl border border-white/15 bg-black p-4 transition-transform hover:-translate-y-0.5 hover:border-white/35"><Icon className="mb-5 size-5 text-[var(--purple-light)]" /><p className="text-sm font-bold">{category}</p><p className="mt-1 text-xs text-white/40">{count} {count === 1 ? "quote" : "quotes"}</p></Link>;
              })}
            </div>
          </section>
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              {collections.toSorted((a, b) => a.displayOrder - b.displayOrder).map((collection) => (
                <Link key={collection.slug} href={`/collections/${collection.slug}`} className="content-card group flex items-center gap-4 rounded-2xl p-5 transition-transform hover:-translate-y-0.5">
                  ...
                  <span className="min-w-0 flex-1"><span className="block font-bold">{collection.title}</span><span className="mt-1 block text-sm text-white/45">{collection.description}</span></span>
                  <ChevronRight className="size-5 text-white/40 transition-transform group-hover:translate-x-1" />
                </Link>
              ))}
            </div>
          </section>
