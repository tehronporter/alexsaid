    ...
    <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
      <Card className="content-card overflow-hidden">
        ...
        <CardContent className="space-y-4 p-6">
          {[{ icon: Smartphone, text: "Launches like an app from your Home Screen" }, { icon: WifiOff, text: "Keeps the core quote experience available offline" }, { icon: Bell, text: "Ready for daily Web Push when delivery is activated" }].map(({ icon: Icon, text }) => <div key={text} className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-white/10"><Icon className="size-4" /></span><span className="text-sm">{text}</span></div>)}
        </CardContent>
      </Card>
      <div className="space-y-5">
        ...
        <Alert className="border-white/15 bg-black text-white"><Bell /><AlertTitle>Daily notifications are not active yet.</AlertTitle><AlertDescription className="text-white/55">{pushEnabled ? "Delivery setup is available after you opt in." : "The permission and exact-quote routing contract is built, but scheduled delivery stays disabled until the secure backend is connected."}</AlertDescription></Alert>
        <Button variant="outline" disabled={!pushEnabled} className="w-full border-white/15 bg-black text-white hover:bg-white hover:text-black" onClick={() => trackProductEvent("notification_interest")}>Notify me daily</Button>
      </div>
    </div>
  );
}
